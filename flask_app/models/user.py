...
class User:
    ...
    @classmethod
    def get_by_id(cls, data): #data es una variable podria llamarse pollino
        #data = {"id":1}
        query = "SELECT * FROM users WHERE id = %(id)s"
        result = connectToMySQL('foro_publicaciones').query_db(query,data) #lista con 1 diccionario
        user = cls(result[0])
        return user