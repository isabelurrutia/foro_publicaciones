...
def register():
    ...
    session["posts.user_id"] = id
    return redirect("/dashboard")

@app.route("/dashboard")
def dashboard():
    #Verificar que el usuario haya iniciado sesión
    if 'posts.user_id' not in session:
        return redirect("/")
    
    dicc= {'id':session['posts.user_id']}
    ...
    return render_template("dashboard.html", user=user, posts=posts, comments=comments)
...
def login():
    ...
    session["posts.user_id"] = user.id
    session["comments.user_id"]= user.id
    return redirect("/dashboard")
